...
def profile():
    ...
    if request.method == 'POST':
        ...
        return render_template("edit_profile.html", nickname=session['user_name'], email=session['email'], passwd=session['password'], friends=session['friends'], all_users=get_all_users(session['email']))
...
def process_signup():
    faltan = []
    campos = ['nickname', 'email', 'passwd', 'passwd_confirmation', 'signup_submit']
    for campo in campos:
    ...
    return create_user_file(request.form['nickname'], request.form['email'], request.form['passwd'], request.form['passwd_confirmation'])
...
def load_user(email, passwd):
    ...
    if not os.path.isfile(file_path):
        return process_error("User not found", url_for("login"))
    with open(file_path, 'r') as f:
        data = json.load(f)
    if data['password'] != passwd:
        return process_error("Incorrect password", url_for("login"))
...
def create_user_file(name, email, passwd, passwd_confirmation):
    ...
    if os.path.isfile(file_path):
        return process_error("The email is already used, you must select a different email", url_for("signup"))
    if passwd != passwd_confirmation:
        return process_error("Your password and confirmation password do not match", url_for("signup"))
    datos = {
        "user_name": name,
        "password": passwd,
        "messages": [],
